...
def main(data_dir: Path):
    ...
    agglomerative_labels = agglomerative.fit_predict(seg_scaler.transform(seg_raw))
    ...
    segment_silhouette = float(
        silhouette_score(seg_scaler.transform(seg_raw), seg_labels)
    )
    segment_stats = {}
    for cluster, tier in cluster_to_tier.items():
        subset = train_fe.loc[seg_labels == cluster]
        segment_stats[tier] = {
            "cluster": int(cluster),
            "count": int(len(subset)),
            "mean_price": float(subset[TARGET].mean()),
            "median_price": float(subset[TARGET].median()),
            "mean_living_area": float(subset["GrLivArea"].mean()),
            "mean_overall_quality": float(subset["OverallQual"].mean()),
            "mean_house_age": float(subset["HouseAge"].mean()),
        }
    segment_points = pd.DataFrame({
        "lot_area": train_fe["LotArea"],
        "sale_price": train_fe[TARGET],
        "cluster": seg_labels,
    }).sample(n=min(500, len(train_fe)), random_state=RANDOM_STATE)
    agglomerative_points = pd.DataFrame({
        "lot_area": train_fe["LotArea"],
        "sale_price": train_fe[TARGET],
        "cluster": agglomerative_labels,
    }).sample(n=min(500, len(train_fe)), random_state=RANDOM_STATE)
    dendro_sample = seg_raw.sample(n=min(90, len(seg_raw)), random_state=RANDOM_STATE)
    dendro_linkage = linkage(
        seg_scaler.transform(dendro_sample), method="ward", metric="euclidean"
    )
    dendro = dendrogram(dendro_linkage, no_plot=True, color_threshold=None)
    dendrogram_lines = []
    dendro_max = float(max(dendro_linkage[:, 2].max(), 1.0))
    dendro_x_max = max(max(xs) for xs in dendro["icoord"])
    dendro_cut_height = float(
        (np.sort(dendro_linkage[:, 2])[-3] + np.sort(dendro_linkage[:, 2])[-2]) / 2
    )
    dendro_height = 320
    for xs, ys in zip(dendro["icoord"], dendro["dcoord"]):
        points = " ".join(
            f"{x / dendro_x_max * 1000:.1f},{(dendro_max - y) / dendro_max * 280 + 20:.1f}"
            for x, y in zip(xs, ys)
        )
        dendrogram_lines.append(points)
    dendro_cut_y = (dendro_max - dendro_cut_height) / dendro_max * 280 + 20
    ...
    required_schema = raw_source.columns.tolist()
    input_dtypes = {c: str(raw_source[c].dtype) for c in required_schema}
    ...
    with open(ART / "segment_meta.json", "w") as f:
        json.dump({
            "n_clusters": int(km.n_clusters),
            "features": SEGMENT_FEATURES,
            "silhouette_score": segment_silhouette,
            "cluster_to_tier": cluster_to_tier,
            "segment_stats": segment_stats,
            "points": segment_points.to_dict(orient="records"),
            "agglomerative_points": agglomerative_points.to_dict(orient="records"),
            "dendrogram": {
                "lines": dendrogram_lines,
                "width": 1000,
